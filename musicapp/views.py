...
def telugu_songs(request):
    """
    View function to retrieve and display Telugu songs.
    """
    # Assuming TeluguSong is your model for Telugu songs
    telugu_songs = TeluguSong.objects.all()

    # Serialize the queryset to JSON format
    serialized_songs = serialize_telugu_songs(telugu_songs)

    return JsonResponse(serialized_songs, safe=False)

def serialize_telugu_songs(songs):
    """
    Helper function to serialize Telugu song objects.
    """
    serialized_songs = []
    for song in songs:
        serialized_song = {
            ...
            'title': song.title,
            'artist': song.artist,
            'song_img': song.song_img.url,  # Assuming you have an image field for song images
